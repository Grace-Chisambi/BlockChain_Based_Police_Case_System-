...
document.getElementById('closureForm')?.addEventListener('submit', async function(e) {
    ...
    try {
        const response = await fetch("{{ route('investigator.case_closures.store') }}", {
            method: "POST",
            headers: {
        ...
        if (!response.ok || !data.success) {
            ...
            if (data.transaction_hash) {
                ...
                showAlert('success', `Case closed successfully.<br><small>Blockchain logging failed or skipped.</small>`);
            }
